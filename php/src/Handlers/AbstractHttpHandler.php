...
abstract class AbstractHttpHandler
{
    ...
    protected function calculateDiscountedFare(
        ...
        if ($ride !== null) {
            ...
            // 初回利用クーポンを最優先で使う
            $stmt = $db->prepare(
                'SELECT discount FROM coupons WHERE user_id = ? AND code = \'CP_NEW2024\' AND used_by IS NULL'
            );
            $stmt->execute([$userId]);
            $coupon = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            if ($coupon === false) {
                $stmt = $db->prepare(
                    'SELECT discount FROM coupons WHERE user_id = ? AND used_by IS NULL ORDER BY created_at LIMIT 1'
                );
                $stmt->execute([$userId]);
                $coupon = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($coupon !== false) {
